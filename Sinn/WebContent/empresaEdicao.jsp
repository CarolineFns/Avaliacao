				...
				<form class="form-horizontal" method="post" action="EmpresaServlet" name="form1">
				    ...
				    <div class="form-group">
				      ...
				      <div class="col-sm-10">
				      	...
				      	onKeyPress="mascaraCNPJ(form1.cnpj);" maxlength="18" onkeydown="mascaraCNPJ(form1.cnpj);" required>
				      </div>
				    </div>
					  	...
					  	<div class="col-sm-10">
						  	<select class="form-control" id="dono" name="dono">
							    ...
							    <c:forEach var="pessoa" items="${listaPessoa}">
                                	<option value="${pessoa.id}" ${pessoa.id eq empresa.dono.id ? "SELECTED" : ""} >${pessoa.nome}</option>
                            	</c:forEach>
						  	</select>
						  </div>
					</div>
